...
<section id="splash-section" style="height: 100vh;" class="d-flex flex-column justify-content-center align-items-center text-black" v-if="content">
  <b-row class="mt-auto w-100">
    <b-col cols="12" class="text-center">
      ...
      <!-- <div class="mt-5"><h1>{{content.splashtext[0].text}}</h1></div> -->
      <div class="mt-5 description-text">
        <p class="text-center">{{content.splashtext[1].text}}</p>
      </div>
      <div class="mt-5">
        <SquareButton @clickButton="$router.push('/home')" :theme="'light'" :label1="'Enter Website'" :icon="'arrow-right'"/>
      </div>
    </b-col>
  </b-row>
  <div class="footer-break-line w-100 mt-auto" :style="'background-image: url(' + getBreakLine + ')'"></div>
</section>
</template>
...
#splash-section .bottom-logo-container {
  text-align: right;
  margin-top: auto;
  margin-bottom: 24px;
}

#splash-section .description-text {
  margin: 0 auto;
  max-width: 475px;
  padding: 0 20px;
}
