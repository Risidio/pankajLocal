...
<div class="p-2">
  <div class="row ml-4 mt-3 border-bottom mb-3 pb-2" v-for="(gaiaAsset, index) in getGaiaAssets" :key="index">
    <div class="col-2 my-2">
      <div><img width="70px" :src="gaiaAsset.attributes.artworkFile.fileUrl"/></div>
    </div>
    ...
    <div class="col-10" v-if="gaiaAsset.contractAsset.beneficiaries">
      <div class="row" v-for="(beneficiary, index) in gaiaAsset.contractAsset.beneficiaries" :key="index">
        ...
        <div class="col-2">{{beneficiary.secondaryRoyalty}}</div>
        <div class="col-6">{{beneficiary.chainAddress}}</div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    formatDate: function (date) {
      const dt = DateTime.fromMillis(date).toLocaleString({ weekday: 'short', month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' })
      return dt
    },
